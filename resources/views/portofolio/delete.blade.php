        ...
        <div class="card-table">
            <div class="container mt-3 px-2">
              ...
              <tbody>
                ...
                @foreach($data as $j)
                <tr>
                  <th scope="row"><input class="form-check-input" value="{{ $j->id }}" id="id[]" name="checkItem" type="checkbox"></th>
                  <td>{{ $j->judul }}</td>
                  <td>{{ $j->tech_stack }}</td>
                  <td><a href="{{ $j->link }}" style="text-decoration: none;">{{ $j->link }}</a></td>
                  <td>{{ $j->durasi }} Hari</td>
                  <td>{{ $data2->where('id',$j->id_category)->first()->nama }}</td>
                  <td class="div-end"><a class="card-button-update" href="{{ route('portofolio.edit', $j->id) }}">
              ...
              <div class="container" style="justify-content: end; align-items:center;display:flex;">
               <a href="{{ route('portofolio.delete') }}">
